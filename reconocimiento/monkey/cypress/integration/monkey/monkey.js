...
const url =
  Cypress.config("baseUrl") || "http://localhost:2368/ghost/#/dashboard";
const urlsToTest = [
  "http://localhost:2368/ghost/#/tags",
  "http://localhost:2368/ghost/#/members",
  "http://localhost:2368/ghost/#/dashboard",
  "http://localhost:2368/ghost/#/site",
  "http://localhost:2368/ghost/#/explore",
...
function avPag() {
  ...
  if (curPageMaxY - curY >= viewportHeight) {
    if (curPageMaxY - (curY + viewportHeight) >= viewportHeight) {
      ...
      //cy.scrollTo(curX, curY, { duration: 100 });
    } else {
...
describe(`${appName} under monkeys`, function () {
  ...
  it(`visits ${appName} and survives monkeys`, function () {
    ...
    if (pcg === 100) {
      ...
      cy.get("#identification").type("pruebas_automaticas@gmail.com");
      cy.get("#password").type("Uniandes2023");
      cy.get("#ember5").click();
      cy.wait(4000);
